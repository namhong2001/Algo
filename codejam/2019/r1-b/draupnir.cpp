...
BigInt cal(BigInt a, BigInt b, int cnt) {
    for (int i=0; i<cnt; ++i) {
        a -= b;
        if(a < 0) a += mod;
    }
...
int main() {
    ...
    for (int t=1; t<=T; ++t) {
        ...
        for (int i=1; i<=6; ++i) {
            ...
            string r;
            cin >> r;
            resp.push_back(BigInt(r));
        }
        resp[0] /= 2;
        resp[1] = cal(resp[1], resp[0], 4) / 2;
        resp[2] = cal(resp[2], resp[0], 8) / 2;
        resp[3] = cal(cal(resp[3], resp[0], 16), resp[1], 4) / 2;
        resp[4] = cal(resp[4], resp[0], 32) / 2;
        resp[5] = cal(cal(cal(resp[5], resp[0], 64), resp[1], 8), resp[2], 4) / 2;
